...
// ============================================================================
export function findIndentPair (doc: vscode.TextDocument, startLnum: number, pair: string[]) {
  let i;
...
// ============================================================================
export function analyzeIndentLevelBackward (doc: vscode.TextDocument, curLnum: number) {
  let i;
  let cntIndent     = 0;
  let cntList       = 0;
  let anchorLnum    = 0;
  ...
  if (indentPair != null) {
      ...
      for (i = curLnum - 1; i >= 0; --i) {
          ...
          if (isValidLine(line)) sentenceExsit = true;
      }
  }
...
export function getAlignBoundary (lnumStart: number, fmtCur: SentenceFormat) {
  ...
  for (var startBound = lnumStart - 1; startBound >= 0; --startBound) {
      ...
      if (fmtCur.type == "typedef" && fmtTmp.broken) break;
      if (reIndentEntry.test(line))                  break;
      if (reIndentExit.test(line))                   break;
      if (reIndentSingle.test(line))                 break;
      if (fmtCur.type != fmtTmp.type)               break;
...
export function alignOpenParen()   { alignRegExp("\\("); }
export function alignLineComment() { alignRegExp("\\/\\/"); }
export function alignEqual()       { alignRegExp("="); }
...
export function convPortDeclToConnImplicit() { convPortDeclToConn("implicit"); }
...
export function convSignalDeclToBReset() { convSignalDeclToReset("blocking"); }
