...
const fs = require("fs");
const lserv = require("vscode-languageserver");
const sv_analyzer = require("../sv_parser/sv_analyzer");
...
connection.onDidChangeConfiguration((_didChangeConfigurationParams) => {
    ...
    for (let filelist of filelists) {
        ...
        for (let filename of filenames) {
            ...
            serverlog(filename);
            let uri = vscode_uri_1.default.file(filename);
            let text;
            try {
...
connection.onExecuteCommand((_executeCommandParams) => {
    ...
    switch (_executeCommandParams.command) {
        case "sv.server.instantiateModule":
            let uri = param.arguments[0];
            let rng = param.arguments[1];
