...
const pg = require("antlr4ts");
const pgtree = require("antlr4ts/tree");
const svlex = require("./antlr_out/SystemVerilogLexer");
const svpar = require("./antlr_out/SystemVerilogParser");
const sv_listener = require("./sv_listener");
const sv_pp = require("./sv_preprocessor");
const sv_error = require("./sv_error");
...
class sv_analyzer {
    ...
    //////////////////////////////////////////////////////////////////////////////
    convPositionDstToSrc(repl_tbl, line, column) {
        let new_line = line;
        let new_char = column;
    ...
    convPositionSrcToDst(repl_tbl, line, column) {
        let new_line = line;
        let new_char = column;
        for (let repl of repl_tbl) {
    ...
    getDataDeclElemStr(elem, defstr = "", usedef = false) {
        if (elem) {
    ...
    getDataDeclStr(decl, print_for = null) {
        let str = "";
    ...
    enumSearchingModules(cur_uri, current_scopes, modifier_scope) {
        let mods = [];
    ...
    searchDeclsOfVariable(refmods, root_type, ident, partial = false) {
        let decls = [];
        function matchIdent(dict, ident) {
            if (partial) {
                for (let key in dict) {
                    let id = (ident) ? ident : "";
                    if (key.indexOf(id) >= 0) {
    ...
    searchModules(uri, current_scope, modifier_scope, ident, modclass = null, partial = false) {
        let mods = [];
    ...
    searchModule(uri, current_scope, modifier_scope, ident, modclass = null) {
        return this.searchModules(uri, current_scope, modifier_scope, ident, modclass, false)[0];
    }
    searchTypeOfDecl(decl) {
        if (!(decl && decl.user))
            return null;
    ...
    resolveHierReference(refmods, root_type, hiers) {
        if (!(hiers && hiers[0]))
            return root_type;
        let ident = hiers[0];
    ...
    getSimpleIdent(input, line, column) {
    ...
    getScopedIdent(input, line, column) {
        let text = input.split("\n");
        --column;
        let i;
        for (i = column; i >= 0; --i) {
    ...
    /////////////////////////////////////////////////////////////////////////////
    getInstantiateModule(uri, ident, line, char, base_indent) {
...
exports.sv_analyzer = sv_analyzer;
//# sourceMappingURL=sv_analyzer.js.map